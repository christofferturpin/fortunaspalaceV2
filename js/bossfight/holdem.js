...
(function (global) {
  ...
  function blindsFor(handNo) {
    // Hands 1–5:   SB 1  / BB 2
    // Hands 6–10:  SB 5  / BB 10
    // Hands 11–20: SB 10 / BB 20
    // Hand 21+:    every 10-hand block adds 10 to SB (BB = 2×SB).
    //   21–30: 20/40,  31–40: 30/60,  41–50: 40/80, …
    if (handNo <= 5)  return { sb: 1,  bb: 2  };
    if (handNo <= 10) return { sb: 5,  bb: 10 };
    if (handNo <= 20) return { sb: 10, bb: 20 };
    const block = Math.floor((handNo - 21) / 10);
    const sb = 20 + block * 10;
    return { sb, bb: sb * 2 };
  }
  ...
  function legalActions(s) {
    ...
    // Effective-stack cap: in heads-up with no side pots, you can't put in
    // chips your opponent can't cover. Otherwise the excess never gets
    // refunded, bets never match, and the betting round never closes —
    // showdown skipped, pot stranded, splash never fires.
    const effMax = Math.min(myStack, oppStack + (oppBet - myBet));
    const minBet = Math.min(effMax, s.minRaise);
    const minRaiseTo = Math.min(myBet + effMax, oppBet + s.minRaise);
    const maxRaiseTo = myBet + effMax;
    const maxBet = effMax;
    return {
  ...
  function advanceStreet(s) {
    ...
    // Post-flop: non-button (BB) acts first in heads-up. When both are all-in
    // there are no chips behind, so the page driver issues an auto-'check'
    // each tick — that's what walks the runout one street at a time and lets
    // the equity badge update between cards.
    s.toAct = s.buttonOnPlayer ? 'chavez' : 'player';
  }

  // ───── Equity (player win % vs Chavez given current visible state) ─────
  // Used by the page during all-in runout to show the player their odds
  // between each card. Monte Carlo over remaining-deck completions; cheap
  // (~600 samples × 5-card eval is well under a frame).
  function equity(s, samples) {
    if (!s.playerHole || s.playerHole.length < 2) return null;
    if (!s.chavezHole || s.chavezHole.length < 2) return null;
    const need = 5 - s.board.length;
    if (need <= 0) {
      const me   = evalBest(s.playerHole.concat(s.board));
      const them = evalBest(s.chavezHole.concat(s.board));
      const c = compare(me, them);
      return c > 0 ? 1 : c < 0 ? 0 : 0.5;
    }
    const known = new Set();
    function k(c) { return c.rank + '_' + c.suit; }
    s.playerHole.forEach(c => known.add(k(c)));
    s.chavezHole.forEach(c => known.add(k(c)));
    s.board.forEach(c => known.add(k(c)));
    const pool = [];
    for (const su of SUITS) {
      for (let r = 2; r <= 14; r++) {
        const c = makeCard(r, su);
        if (!known.has(k(c))) pool.push(c);
      }
    }
    const N = samples || 600;
    let wins = 0, ties = 0;
    const baseBoard = s.board.slice();
    for (let i = 0; i < N; i++) {
      // Partial Fisher-Yates: pull `need` distinct cards from pool
      const picks = [];
      const used = new Array(pool.length);
      for (let j = 0; j < need; j++) {
        let idx;
        do { idx = (Math.random() * pool.length) | 0; } while (used[idx]);
        used[idx] = true;
        picks.push(pool[idx]);
      }
      const full = baseBoard.concat(picks);
      const me   = evalBest(s.playerHole.concat(full));
      const them = evalBest(s.chavezHole.concat(full));
      const c = compare(me, them);
      if (c > 0) wins++;
      else if (c === 0) ties++;
    }
