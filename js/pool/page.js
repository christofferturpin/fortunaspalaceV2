...
(function () {
  ...
  // ⇧-modifier gain: well below normal so the witch can trace single-pixel
  // curves. Drag re-anchors on shift toggle so the handle never snaps.
  const HANDLE_SENS_PRECISE = 0.12;
  ...
  const state = {
    ...
    dragging: false,
    dragShift: false,         // tracks the precision modifier so onMove can re-anchor
    lastT: 0,
  ...
  function init() {
    ...
    state.canvas.addEventListener('mousemove', onMove, { passive: true });
  ...
  function onMove(e) {
    ...
    if (state.phase === 'AIM') {
      ...
      // Hold ⇧ to drop the gain to HANDLE_SENS_PRECISE for super-fine bends.
      // Re-anchor on every modifier flip so the handle position doesn't snap
      // when the player taps shift mid-drag.
      const shiftHeld = !!e.shiftKey;
      if (shiftHeld !== state.dragShift) {
        ...
        state.dragShift = shiftHeld;
      }
      const sens = shiftHeld ? HANDLE_SENS_PRECISE : HANDLE_SENS;
      const dx = p.x - state.dragAnchorMouse.x;
      const dy = p.y - state.dragAnchorMouse.y;
      state.handle.x = state.dragAnchorHandle.x + dx * sens;
      state.handle.y = state.dragAnchorHandle.y + dy * sens;
      clampHandle();
    }
  }
  ...
  function onUp() { state.dragging = false; state.dragShift = false; }

  // Keep the bend handle on the felt so it never gets stuck under the header
  // or the page chrome where the player can't grab it. Also cap how close to
  // the cue ball the handle can be pulled — inside HANDLE_DIST * 0.5 the
  // bend collapses into a straight shot and trivializes the chain mechanic.
  // The cap is a radius around the cue (free to rotate, can't pull in past
  // the ring), not a half-plane cone along naturalDir.
  function clampHandle() {
    ...
    const minRadius = HANDLE_DIST * 0.5;
    const hx = state.handle.x - state.cue.x;
    const hy = state.handle.y - state.cue.y;
    const dist = Math.hypot(hx, hy);
    if (dist < minRadius) {
      let dx, dy;
      if (dist < 1e-6) {
        const nd = state.naturalDir;
        dx = (nd && (nd.x || nd.y)) ? nd.x : 1;
        dy = (nd && (nd.x || nd.y)) ? nd.y : 0;
      } else {
        dx = hx / dist;
        dy = hy / dist;
      }
      state.handle.x = state.cue.x + dx * minRadius;
      state.handle.y = state.cue.y + dy * minRadius;
      // The push outward can land the handle off-felt when the cue is
      // hugging a rail. Re-clamp so the handle stays grabbable — the radius
      // becomes a soft preference in that degenerate corner.
      clampToFelt();
    }
  }
  ...
  function updateUI() {
    ...
    // Coaching prompt — swaps text per phase. Hidden outside AIM/BEND so it
    // doesn't compete with the action mid-flight.
    const pressSpace = document.getElementById('pool-press-space');
    if (pressSpace) {
      const aimOn = state.phase === 'AIM';
      const bendOn = state.phase === 'BEND';
      pressSpace.classList.toggle('is-on', aimOn || bendOn);
      if (aimOn) pressSpace.textContent = 'aim with mouse and press space, don’t forget to adjust power';
      else if (bendOn) pressSpace.textContent = 'now bend by grabbing the handle and press space';
    }
    switch (state.phase) {
      case 'AIM':
      ...
      case 'FIRED':
        ...
        break;
      case 'BEND':
        status.textContent = 'Bend the carom — drag the binding (hold ⇧ for fine), then RELEASE.';
