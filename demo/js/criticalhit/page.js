/*
  ┌──────────────────────────────────────────────────────────────────────────┐
  │  CRITICAL HIT — wagered dice-merging round (10d4, threshold 8)           │
  ├──────────────────────────────────────────────────────────────────────────┤
  │                                                                          │
  ...
  │     initDice (10 d4 in dice pool); rollAllFree (initial tumble)          │
  │                                                                          │
  ...
  │                                                                          │
  │   Payout tiers (require score ≥ 22 to win anything):                     │
  │     ≤21        lose wager                                                │
  │     22–29      wager × 1   (PUFF — refund)                               │
  │     30–41      wager × 2   (HIT)                                         │
  │     42–62      wager × 3   (BIG HIT)                                     │
  │     63+        wager × 6   (CRITICAL HIT)                                │
  │                                                                          │
  ...
  SIZES=[4,6,8,10,12,20]; SCORE_BY_SIZE={4:0,6:3,8:7,10:15,12:31,20:63}
  START_COUNT=10; START_SIZE=4; START_REROLLS=2; MIN_WIN_SCORE=22
  PAYOUT_TIERS=[{min:63,mult:6},{min:42,mult:3},{min:30,mult:2},{min:22,mult:1}]
  TICK_MS=55; TICKS=9
...
(function (global) {
  ...
  // d_n = 2^n − 1 series. Guarantees nextDie > 2 × prevDie, so a single
  // larger die is always worth strictly more than two of the smaller size.
  // Same-ladder merge is exactly +1 net; cross-size matches (a d10 face
  // matching a d6 face) are the big plays — they can net +13 or more.
  const SCORE_BY_SIZE = { 4: 0, 6: 3, 8: 7, 10: 15, 12: 31, 20: 63 };
  const START_COUNT = 10;
  const START_SIZE = 4;
  const START_REROLLS = 2;
  const MIN_WIN_SCORE = 22;
  const TICK_MS = 55;
  const TICKS = 9;

  // Highest tier first — payoutFor() scans top-down. Tightened: PUFF is
  // harder to clear, and the top tier no longer doubles your bankroll on a
  // single round. A d20 alone (63) still hits CRITICAL but pays ×6 instead
  // of the old ×20.
  const PAYOUT_TIERS = [
    { min: 63, mult:  6, label: 'CRITICAL HIT' },
    { min: 42, mult:  3, label: 'BIG HIT'      },
    { min: 30, mult:  2, label: 'HIT'          },
    { min: 22, mult:  1, label: 'PUFF'         },
  ];
        ...
        if (detailEl) {
          if (state.payout <= 0) {
            detailEl.textContent =
              `Score ${s} · need ${MIN_WIN_SCORE} to break even. Wager (${state.wager}) burned.`;
          } else if (t.mult === 1) {
            detailEl.textContent =
              `Score ${s} · wager (${state.wager}) refunded.`;
          } else {
            detailEl.textContent =
